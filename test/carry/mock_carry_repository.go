...
func (m *MockCarryRepository) Create(v models.Carry) (w models.Carry, err error) {
	args := m.Called(v)
	return args.Get(0).(models.Carry), args.Error(1)
}

func (m *MockCarryRepository) SearchByLocality(locality_id int) (v map[string]models.CarryByLocality, err error) {
	args := m.Called()
